...
import { VALIDATION_LIMITS, formatCurrency } from "../utils/validationConfig";
...
export default function CreditForm() {
  ...
  const { cedulaLiveError, handleCedulaChange, clearCedulaError } = useCedulaValidation(setValue, trigger);
  const { montoDisplay, montoError, handleMontoChange, handleMontoBlur, handleMontoFocus,
          salarioDisplay, salarioError, handleSalarioChange, handleSalarioBlur, handleSalarioFocus,
          clearErrors: clearNumericErrors } = useNumericInput(setValue, watch);
  ...
  const inputClass = (hasError: boolean, isValid: boolean) =>
    `input-field ${hasError ? "input-field-error" : isValid ? "border-green-400 focus:border-green-500 focus:ring-green-500/20" : ""}`;
    ...
    <div className="card animate-fade-in">
      <div className="px-5 sm:px-7 py-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold text-gray-900">Nueva Evaluación</h2>
            <p className="text-xs text-gray-500">Complete los datos del solicitante</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="px-5 sm:px-7 py-5 space-y-5">
        {/* Cédula */}
        <div>
          <label htmlFor="cedula" className="block text-sm font-medium text-gray-700 mb-1.5">Cédula de identidad</label>
          <div className="relative">
            ...
            <input
              ...
              autoComplete="off"
            />
            {isCedulaValid && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500"><svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" /></svg></span>}
          </div>
          <div className="mt-1.5 flex items-center justify-between">
            {cedulaError ? (
              <p id="cedula-error" className="text-sm text-red-600 flex items-center gap-1" role="alert">
                <svg className="h-4 w-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-5a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0v-4.5A.75.75 0 0110 5zm0 10a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" /></svg>
                {cedulaError}
              </p>
            ) : (
              <p id="cedula-hint" className="text-xs text-gray-400">
                {cedulaDigits < 10 ? `${cedulaDigits}/10 dígitos` : "Cédula válida"}
              </p>
            )}
            <div className="h-1 w-20 rounded-full bg-gray-100 overflow-hidden ml-2">
              <div className={`h-full rounded-full transition-all duration-300 ${cedulaError ? "bg-red-400" : isCedulaValid ? "bg-green-400" : "bg-blue-400"}`} style={{ width: `${cedulaProgress}%` }} />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            <label htmlFor="montoSolicitado" className="block text-sm font-medium text-gray-700 mb-1.5">Monto solicitado (USD)</label>
            <div className="relative">
              ...
              <input
                ...
                autoComplete="off"
              />
              {isMontoValid && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500"><svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" /></svg></span>}
            </div>
            {montoError ? (
              <p id="monto-error" className="mt-1.5 text-sm text-red-600">{montoError}</p>
            ) : (
              <p id="monto-hint" className="mt-1.5 text-xs text-gray-400">Rango: ${formatCurrency(MONTO.MIN)} - ${formatCurrency(MONTO.MAX)}</p>
            )}
          </div>

          <div>
            <label htmlFor="salario" className="block text-sm font-medium text-gray-700 mb-1.5">Salario mensual (USD)</label>
            <div className="relative">
              <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-medium">$</span>
              <input
                id="salario"
                ...
                autoComplete="off"
              />
              {isSalarioValid && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500"><svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" /></svg></span>}
            </div>
            {salarioError ? (
              <p id="salario-error" className="mt-1.5 text-sm text-red-600">{salarioError}</p>
            ) : (
              <p id="salario-hint" className="mt-1.5 text-xs text-gray-400">Rango: ${formatCurrency(SALARIO.MIN)} - ${formatCurrency(SALARIO.MAX)}</p>
            )}
          </div>
        </div>
        ...
        {/* Plazo */}
        <div>
          <label htmlFor="tiempoAnios" className="block text-sm font-medium text-gray-700 mb-1.5">Plazo del crédito (años)</label>
          <div className="relative">
            <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
            ...
            <input
              ...
              type="text"
              inputMode="numeric"
              {...register("tiempoAnios", { setValueAs: (v: string) => { const n = parseInt(v, 10); return isNaN(n) ? undefined : n; } })}
              ...
              autoComplete="off"
            />
            {isPlazoValid && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500"><svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" /></svg></span>}
          </div>
          {errors.tiempoAnios ? <p id="plazo-error" className="mt-1.5 text-sm text-red-600">{errors.tiempoAnios.message}</p> : <p id="plazo-hint" className="mt-1.5 text-xs text-gray-400">Rango: {PLAZO.MIN} - {PLAZO.MAX} años</p>}
        </div>

        {/* Submit */}
        <button type="submit" disabled={mutation.isPending || hasFieldErrors || hasIncompleteFields} className="btn-primary w-full">
          {mutation.isPending ? (
            <>
        ...
        {mutation.isError && (
          <div className="animate-slide-up rounded-lg bg-red-50 border border-red-200 p-4" role="alert">
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-sm font-semibold text-red-800">Error al evaluar</p><p className="text-sm text-red-600 mt-0.5">Verifique los datos e intente nuevamente.</p></div>
            </div>
          </div>
        )}
        ...
        {/* Mutation success */}
        {mutation.isSuccess && (
          <div className={`animate-slide-up rounded-lg border p-4 ${mutation.data.estado === "APROBADO" ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}>
            <div className="flex items-start gap-3">
              {mutation.data.estado === "APROBADO" ? (
                <svg className="h-5 w-5 text-emerald-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" /></svg>
              ) : (
                <svg className="h-5 w-5 text-red-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z" clipRule="evenodd" /></svg>
              )}
              <div className="flex-1">
                <p className={`text-sm font-semibold ${mutation.data.estado === "APROBADO" ? "text-emerald-800" : "text-red-800"}`}>Resultado: {mutation.data.estado}</p>
                <div className="mt-1.5 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                  <div><span className="text-gray-500">Score:</span> <span className="font-semibold text-gray-900">{mutation.data.score}</span></div>
                  <div><span className="text-gray-500">Deuda mensual:</span> <span className="font-semibold text-gray-900">${mutation.data.deudaMensual?.toFixed(2)}</span></div>
                </div>
              </div>
            </div>
          </div>
